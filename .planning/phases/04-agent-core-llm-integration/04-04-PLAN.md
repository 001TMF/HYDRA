  ...
  <action>
    ...
    **Create `src/hydra/agent/arena.py`**:
    ...
    - `Arena`:
      ...
      - `check_promotion(self, arena_results: list[ArenaResult], champion_fitness: float) -> PromotionResult | None`: For the top-ranked ArenaResult, if it beats champion_fitness, run full 5-window promotion evaluation. This is a simplified version that compares the top candidate's fitness against champion across the arena's internal evaluation windows. Returns PromotionResult from PromotionEvaluator.
        - For now, use a simplified approach: the arena stores the fitness_score per hypothesis. The full 5-window evaluation will use actual replay windows in Phase 5 integration. Here, simulate by checking if top candidate fitness > champion_fitness and return a PromotionResult with windows_won estimated from the margin.
    ...
    **Create `tests/test_coordinator.py`**:
    ...
    7. test_check_promotion_returns_result: Top candidate beats champion -> returns PromotionResult.

    Mock ExperimentRunner.run_experiment to return ExperimentResult with synthetic metrics. Mock CompositeEvaluator.score() to return configurable FitnessScore.
  </action>
  <verify>Run `python -m pytest tests/test_coordinator.py -v` -- all 7 tests pass.</verify>
  <done>Head Coordinator dispatches to registered heads, deduplicates, and handles per-head failures gracefully. Arena evaluates hypotheses through experiment runner, ranks by fitness, and checks promotion. 7 tests pass.</done>
</task>
...
2. `python -m pytest tests/test_agent_loop.py tests/test_coordinator.py -v` -- all 13 tests pass
...
<success_criteria>
Agent loop state machine runs full observe-diagnose-hypothesize-experiment-evaluate cycle. Head Coordinator dispatches to multiple heads with per-head failure isolation and dedup filtering. Arena tournament ranks hypotheses by fitness and evaluates top-K for promotion. Abstract BaseHead interface defines the contract for specialized heads. All 13 tests pass.
</success_criteria>
