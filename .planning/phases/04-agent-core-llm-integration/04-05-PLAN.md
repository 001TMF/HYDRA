...
must_haves:
  ...
  key_links:
    ...
    - from: "src/hydra/agent/loop.py"
      ...
      via: "loop calls promotion_evaluator.evaluate(candidate_window_scores, champion_window_scores) when window scores are provided; falls back to single-score comparison when not provided"
...
- `AgentLoop` class:
  ...
  - `run_cycle(self, recent_returns, predictions, actuals, probabilities, baseline_sharpe, baseline_features=None, current_features=None, feature_names=None, current_config=None, champion_fitness=None, candidate_window_scores=None, champion_window_scores=None) -> AgentCycleResult`:
    Main entry point -- runs one complete cycle through all phases. The two optional window score parameters (`candidate_window_scores`, `champion_window_scores`) enable the full 3-of-5 PromotionEvaluator path (AGNT-08). When absent, the method falls back to a simple single-score comparison. Steps:
    ...
    6. **EVALUATE**: Feed current fitness to rollback trigger: `rollback_trigger.update(current_fitness, champion_fitness)`. If rollback fires, call `registry.rollback()` (if method exists), log to journal, return with rolled_back=True. Otherwise, evaluate candidate using the following branching logic:

       ```python
       if candidate_window_scores is not None and champion_window_scores is not None:
           # Full 3-of-5 path (AGNT-08) -- caller provides pre-computed window scores
           ...
           # Honda default: single-score comparison when window scores not available
           # (e.g., early integration, unit tests, or callers that only have one eval point)
           experiment_fitness = experiment_result.fitness_score or 0.0
           promoted = experiment_fitness > (champion_fitness or 0.0)
           promotion_reason = "single-score comparison (no window scores provided)"
       ```

       This makes the full PromotionEvaluator path available (AGNT-08 satisfied) while keeping the simple default functional for all existing tests and callers that do not supply window scores.
...
- Test successful cycle with simple comparison (no window scores): all mocks return positive results -> promoted=True when candidate > champion, journal.log_experiment called
- Test successful cycle with window scores provided: mock promotion_evaluator.evaluate to return PromotionResult(promoted=True, windows_won=3) -> promoted=True, verify promotion_evaluator.evaluate() was called (not skipped)
- Test window scores provided but 3-of-5 fails: mock promotion_evaluator.evaluate to return PromotionResult(promoted=False, windows_won=2) -> promoted=False
