...
<task type="auto">
  ...
  <action>
    ...
    **scripts/plot_bl_density.py** (validation gate diagnostic tool):
    - Standalone script that generates synthetic thin-market options chain data (8-15 strikes with realistic noise)
    - Runs the full SVI -> B-L pipeline: calibrate_svi -> svi_to_call_prices -> extract_density -> compute_moments
    - Produces matplotlib plots saved to `output/` directory:
    ...
    - Prints quality metrics: liquid strike count, density integral, implied mean vs. forward, all four moments
    - Run with: `uv run python scripts/plot_bl_density.py`
    - This script fulfills the roadmap validation gate: "plot implied vs. realized distributions to verify"
    - Add matplotlib to dev dependencies in pyproject.toml if not already present
  </action>
  <verify>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <action>
    ...
    5. Run the B-L validation gate diagnostic: `uv run python scripts/plot_bl_density.py`
       - Inspect the implied density plot in output/ — density should be smooth, unimodal, integrate to ~1.0
       - Compare implied density shape against log-normal benchmark
       - Check printed moments are reasonable (skew near 0 for synthetic data, kurtosis near 3)
