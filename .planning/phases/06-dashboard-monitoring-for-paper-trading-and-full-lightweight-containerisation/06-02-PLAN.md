...
<task type="auto">
  ...
  <action>
    1. Create `docker/Dockerfile`:
       ...
       - Single CMD runs the dashboard; the PaperTradingRunner is integrated as a FastAPI lifespan event (06-03, Task 3). The `HYDRA_START_RUNNER=true` env var in docker-compose.yml activates it.
